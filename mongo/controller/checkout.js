...
async function zaloPayment(products, total, orderDescription, orderInfo) {
    try {
        ...
        order.mac = crypto.createHmac("sha256", key1).update(hmac_input).digest("hex");
        ...
        if (data.return_code === 1) {
            return {status: 200, message: "Tạo dơn hàng (zalo) thành công", zaloResponse: data}
        } else {
            console.log(">>> Tạo đơn hàng (zalo) thất bại: ", data);
            return {status: 500, message: ">>> Tạo đơn hàng (zalo) thất bại", zaloResponse: data}
        }
    } catch (error) {
...
async function handleZaloCallback(body) {
    try {
        ...
        switch (data.return_code) {
            case 1: {
                return {status: 200, message: "Thanh toán thành công", zaloCallback: data};
            }
            case 2: {
                return {
                    status: 200,
            ...
            case 3: {
                return {status: 202, message: "Giao dịch đang xử lí", zaloCallback: data}
            }
            case -1: {
                return {
                    status: 400,
                    message: "Lỗi xác thực từ ZaloPay (MAC sai hoặc thông tin không hợp lệ)",
                    zaloCallback: data
                };
            }
            default: {
